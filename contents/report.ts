...
function getSubmitInfo(document: HTMLDocument) {
  const infoTable = document.querySelectorAll(
    "#listBox .boardListWrite tbody"
  )[1]
  if (!infoTable) {
    return false
  }
  ...
  tempEl.innerHTML = (
    [...infoTable.childNodes].find((el) => el.nodeType == 8) as Comment
  ).data.trim()
  ...
  const submitCnt = Number.parseInt(
    reportInfoDTORaw.match(/submitCnt=([0-9]+),/)[1]
  )
  const lateSubmitCnt = Number.parseInt(
    reportInfoDTORaw.match(/lateSubmitCnt=([0-9]+),/)[1]
  )
  const studentCnt = Number.parseInt(
    reportInfoDTORaw.match(/studentCnt=([0-9]+),/)[1]
  )
...
window.addEventListener("load", () => {
  ...
  const outputTbody = document.querySelectorAll(
    "#listBox .boardListWrite tbody"
  )[0]
  ...
  <td class="last bisue-submit-tr">${submitInfo.submitCnt}(+${
    submitInfo.lateSubmitCnt
  }) / ${submitInfo.studentCnt} (${submitInfo.percent.toFixed(2)}%)</td>
  `
