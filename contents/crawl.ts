...
async function fetchCourses() {
  ...
  const courseRows = dom.querySelectorAll(
    `[name="courseDTO.courseId"] option:not(:first-child)`
  )

  const courseIds = [...courseRows].map(
    (row) => row.getAttribute("value").split(",")[0]
  )

  return courseIds
}

async function fetchNoticesPerPage(
  boardInfoId: string,
  courseId: string,
  page = 1
) {
